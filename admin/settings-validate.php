...
function pressable_cache_management_callback_validate_options($input)
{
    ...
    $input["extend_batcache_checkbox"] = filter_var(
        $input["extend_batcache_checkbox"] == 1 ? 1 : 0,
    ...
    $input["flush_cache_theme_plugin_checkbox"] = filter_var(
        $input["flush_cache_theme_plugin_checkbox"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
    ...
    $input["flush_cache_page_edit_checkbox"] = filter_var(
        $input["flush_cache_page_edit_checkbox"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
    ...
    $input["flush_cache_on_page_post_delete_checkbox"] = filter_var(
        $input["flush_cache_on_page_post_delete_checkbox"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
    ...
    $input["flush_cache_on_comment_delete_checkbox"] = filter_var(
        $input["flush_cache_on_comment_delete_checkbox"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
    ...
    $input["flush_object_cache_for_single_page"] = filter_var(
        $input["flush_object_cache_for_single_page"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
    ...
    $input["cache_wpp_cookies_pages"] = filter_var(
        $input["cache_wpp_cookies_pages"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
    ...
    $input["cache_wpp_cookies_pages"] = filter_var(
        $input["exclude_query_string_gclid_checkbox"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
...
if (isset($input["exempt_from_batcache"])) {
    $input["exempt_from_batcache"] = sanitize_text_field(
        $input["exempt_from_batcache"]
    );
}
...
function cdn_settings_tab_callback_validate_options($input)
{
    ...
    $input["cdn_cache_extender"] = filter_var(
        $input["cdn_cache_extender"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
    ...
    $input["exclude_jpg_png_webp_from_cdn"] = filter_var(
        $input["exclude_jpg_png_webp_from_cdn"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
    ...
    $input["exclude_json_from_cdn"] = filter_var(
        $input["exclude_json_from_cdn"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
    ...
    $input["exclude_css_from_cdn"] = filter_var(
        $input["exclude_css_from_cdn"] == 1 ? 1 : 0,
        FILTER_SANITIZE_NUMBER_INT
    );
...
//     );
    ...
    if (isset($input["exclude_particular_file_from_cdn"])) {
        $input["exclude_particular_file_from_cdn"] = sanitize_text_field(
            $input["exclude_particular_file_from_cdn"]
        );
    }
...
function pressable_api_authentication_tab_callback_validate_options($input)
{
    ...
    if (isset($input["pressable_site_id"])) {
        $input["pressable_site_id"] = sanitize_text_field(
            $input["pressable_site_id"]
        );
    }
    ...
    if (isset($input["api_client_secret"])) {
        $input["capi_client_secret"] = sanitize_text_field(
            $input["api_client_secret"]
        );
    }
...
function remove_pressable_branding_tab_callback_validate_options($input)
{
    ...
    if (
        !array_key_exists(
            $input["branding_on_off_radio_button"],
            $radio_options
        )
    ) {
        $input["branding_on_off_radio_button"] = null;
