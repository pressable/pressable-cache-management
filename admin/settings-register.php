...
function pressable_cache_management_register_settings() {
	...
	*/
	...
	add_settings_field(
		...
		array(
			...
			'label' => esc_html__(
				'Extend the length of time a Batcache render is stored',
				'pressable_cache_management'
			),
		)
	);
	...
	add_settings_field(
		...
		array(
			...
			'label' => esc_html__(
				'Flush batcache automatically on plugin & theme update',
				'pressable_cache_management'
			),
		)
	);
	...
	add_settings_field(
		...
		array(
			...
			'label' => esc_html__(
				'Flush Batcache automatically when new page/post is updated',
				'pressable_cache_management'
			),
		)
	);
	...
	add_settings_field(
		...
		esc_html__( 'Flush Batcache for single page', 'pressable_cache_management' ),
		...
		array(
			...
			'label' => esc_html__(
				'Flush batache cache for a particular page',
				'pressable_cache_management'
			),
		)
	);
	...
	*/
	...
	if ( empty( $api_auth_tab_options['api_client_id'] ) ) {
		add_settings_field(
			'no_cdn_option',
			esc_html__( 'Turn on/off your CDN', 'cdn_settings_tab' ),
			'pressable_cdn_enable_api',
			'cdn_settings_tab',
			'pressable_cache_management_section_cdn',
			array(
				'id'    => 'no_cdn_option',
				'label' => esc_html__( 'Turn on/off your CDN' ),
			)
		);
	} else {
		add_settings_field(
			'cdn_on_off_radio_button',
			esc_html__( 'Turn On/Off CDN', 'cdn_settings_tab' ),
			'pressable_cache_management_callback_field_extend_cdn_radio_button',
			'cdn_settings_tab',
			'pressable_cache_management_section_cdn',
			array(
				'id'    => 'cdn_on_off_radio_button',
				'label' => esc_html__(
					'Turn on/off your CDN',
					'cdn_settings_tab'
				),
			)
		);
	...
	if ( ! empty( $api_auth_tab_options['api_client_id'] ) ) {
		...
		if ( $cdn_tab_options && $cdn_tab_options['cdn_on_off_radio_button'] == 'enable' ) {
			...
			add_settings_field(
				...
				esc_html__( 'Purge CDN Cache', 'cdn_settings_tab' ),
				'pressable_cdn_cache_flush_management_callback_field_button',
				'cdn_settings_tab',
				'pressable_cache_management_section_cdn',
				array(
					'id'    => 'purge_cdn_cache_button',
					'label' => esc_html__(
						'Purge CDN Cache (Static files Img/CSS/JS/WEBP)',
						'cdn_settings_tab'
					),
				)
			);
		...
		if ( $cdn_tab_options && $cdn_tab_options['cdn_on_off_radio_button'] == 'enable' ) {
			...
			add_settings_field(
				...
				array(
					...
					'label' => esc_html__(
						'Extend the cache-control from 7 days until 10 years for static assets',
						'cdn_settings_tab'
					),
				)
			);
	...
	if ( ! empty( $api_auth_tab_options['api_client_id'] ) ) {
		...
		if ( $cdn_tab_options && $cdn_tab_options['cdn_on_off_radio_button'] == 'enable' ) {
			...
			add_settings_field(
				'exclude_jpg_png_webp_from_cdn',
				esc_html__( 'Exlude image file', 'pressable_cache_management' ),
				'pressable_cache_management_callback_field_exclude_cdn_image_webp_checkbox',
				'cdn_settings_tab',
				'pressable_cache_management_section_cdn',
				array(
					'id'    => 'exclude_jpg_png_webp_from_cdn',
					'label' => esc_html__(
						'Exclude .JPG, .PNG, WEBP files from CDN Caching',
						'cdn_settings_tab'
					),
				)
			);
	...
	if ( ! empty( $api_auth_tab_options['api_client_id'] ) ) {
		...
		if ( $cdn_tab_options && $cdn_tab_options['cdn_on_off_radio_button'] == 'enable' ) {
			...
			add_settings_field(
				'exclude_json_js_from_cdn',
				esc_html__( 'Exclude .json and .js file', 'pressable_cache_management' ),
				'pressable_cache_management_callback_field_exclude_json_checkbox',
				'cdn_settings_tab',
				'pressable_cache_management_section_cdn',
				array(
					'id'    => 'exclude_json_js_from_cdn',
					'label' => esc_html__(
						'Exclude all .json and .js files from CDN Caching',
						'cdn_settings_tab'
					),
				)
			);
	...
	if ( ! empty( $api_auth_tab_options['api_client_id'] ) ) {
		...
		if ( $cdn_tab_options && $cdn_tab_options['cdn_on_off_radio_button'] == 'enable' ) {
			...
			add_settings_field(
				'exclude_css_from_cdn',
				esc_html__( 'Exclude .css file', 'pressable_cache_management' ),
				'pressable_cache_management_callback_field_exclude_json_checkbox',
				'cdn_settings_tab',
				'pressable_cache_management_section_cdn',
				array(
					'id'    => 'exclude_css_from_cdn',
					'label' => esc_html__(
						'Exclude all .css files from CDN Caching',
						'cdn_settings_tab'
					),
				)
			);
	...
	if ( ! empty( $api_auth_tab_options['api_client_id'] ) ) {
		...
		if ( $cdn_tab_options && $cdn_tab_options['cdn_on_off_radio_button'] == 'enable' ) {
			...
			add_settings_field(
				...
				esc_html__( 'Exclude a particular file', 'pressable_cache_management' ),
				'pressable_cache_management_callback_field_exclude_partucular_file_text',
				'cdn_settings_tab',
				'pressable_cache_management_section_cdn',
				array(
					'id'    => 'exclude_particular_file_from_cdn',
					'label' => esc_html__(
						'To exclude multiple files separate with | eg template.js|style.css',
						'cdn_settings_tab'
					),
				)
			);
	...
	*/
	...
	if ( $site_id_con_res === 'OK' ) {
		...
		add_settings_field(
			'pressable_site_id',
			esc_html__( 'Site ID', 'pressable_api_authentication_tab' ), //Displayed title
			...
			array(
				'id'    => 'pressable_site_id',
				'class' => 'textbox-hidden',
				'label' => esc_html__( 'Enter the site ID', 'pressable_api_authentication_tab' ),
			) //Array of passed from this field
		);
	...
	add_settings_field(
		'api_client_id',
		esc_html__( 'Client ID', 'pressable_api_authentication_tab' ),
		'pressable_cache_management_callback_field_id_text',
		'pressable_api_authentication_tab',
		'pressable_cache_management_section_api_authentication',
		array(
			'id'    => 'api_client_id',
			'label' => esc_html__(
	...
	add_settings_field(
		'api_client_secret',
		esc_html__( 'Client Secret', 'pressable_api_authentication_tab' ),
