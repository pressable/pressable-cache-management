...
* Version: 2.0.2
* Patched: 2026-09-01 - Added null guards for deleted comments/posts and
...
class Batcache_Manager {
	...
	public function action_clean_post_cache( $post_id ) {
		...
		// --- START OF NULL-GUARD FIX ---
		// clean_post_cache can fire after a post has been permanently deleted,
		// in which case get_post() returns null. Bail early instead of relying
		// on the status check below (which can be fooled by the global post
		// fallback inside get_post_status()).
		if ( ! $post instanceof WP_Post ) {
			return;
		}
		// --- END OF NULL-GUARD FIX ---
	...
	public function action_update_comment( $comment_id ) {
		...
		// --- START OF NULL-GUARD FIX ---
		// wp_delete_comment() removes the row from the database first and only
		// then fires clean_comment_cache and wp_set_comment_status ('delete'),
		// so get_comment() returns null here for every permanently deleted
		// comment (e.g. Akismet spam purges, emptying trash). This was the
		// source of the "Attempt to read property ... on null" warning bursts
		// on lines 232 / 317 / 324 / 325 / 329 / 334 / 335.
	...
	public function setup_term_urls( $term, $taxonomy ) {
		...
		// NULL-GUARD FIX: get_taxonomy() returns false if the taxonomy has been
		// unregistered by this point, so check the object before reading it.
	...
	public function setup_post_urls( $post ) {
		...
		// --- START OF NULL-GUARD FIX ---
		// Bail if the post can't be resolved. This happens when the
		// deleted-comment flow passes a null post ID and there is no global
		// post to fall back on (cron / CLI / REST context), which previously
		// triggered the cascade of "post_type on null" warnings below.
		if ( ! $post instanceof WP_Post ) {
			return;
		}
		// --- END OF NULL-GUARD FIX ---
		...
		// NULL-GUARD FIX: get_post_type_object() returns null for post types
		// that are no longer registered, so check the object before reading it.
		if ( $post_type && $post_type->show_in_rest && $post_type->rest_base ) {
