...
if ( isset( $options['flush_object_cache_for_single_page'] ) && ! empty( $options['flush_object_cache_for_single_page'] ) ) {
	...
	class PcmFlushCacheAdminbar {
		...
		public function add() {
			...
			} elseif ( is_admin() || is_admin_bar_showing() ) {
			...
			add_action(
				'wp_ajax_pcm_purge_current_page_edge_cache',
				array(
					$this,
		...
		function load_toolbar_js() {
			...
			wp_enqueue_script(
				'pcm-toolbar"',
				plugin_dir_url( __DIR__ ) . 'public/js/toolbar.js',
				array(
					'jquery',
				),
				time(),
		...
		function load_remove_branding_toolbar_js() {
			...
			wp_enqueue_script(
				'pcm-toolbar"',
				plugin_dir_url( __DIR__ ) . 'public/js/toolbar_remove_branding.js',
				array(
					'jquery',
				),
				time(),
