...
if (isset($options['exclude_font_files_from_cdn']) && !empty($options['exclude_font_files_from_cdn']))
{
    ...
    $cdn_exclude_font_files = WP_CONTENT_DIR . '/mu-plugins/pcm_exclude_font_files_from_cdn.php';
    if (file_exists($cdn_exclude_font_files)) { 
        ...
        $cdn_exclude_font_files_active = WP_CONTENT_DIR . '/mu-plugins/pcm_exclude_font_files_from_cdn.php';
    ...
    function pcm_exclude_fonts_from_cdn_admin_notice()
    {
        ...
        if ('activating' === $exclude_from_img_cdn_activate_display_notice && current_user_can('manage_options'))
        {
            ...
            add_action('admin_notices', function ()
            {
                ...
                $message = sprintf('<p>Excluded all .WOFF .WOFF2 .OTF .TTF .EOT from CDN caching.</p>', $user->display_name);
