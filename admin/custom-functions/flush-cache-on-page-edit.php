<?php
/**
 * Custom function - Add custom functions to flush cache on page, post and custom post_types on edit.
 *
 * @package Pressable
 */
...
if ( isset( $options['flush_cache_page_edit_checkbox'] ) && ! empty( $options['flush_cache_page_edit_checkbox'] ) ) {
	if ( isset( $options['flush_cache_page_edit_checkbox'] ) && ! empty( $options['flush_cache_page_edit_checkbox'] ) ) {
		/**
		 * Clear batcache on post edit.
		 *
		 * @param int     $post_id The post ID.
		 * @param WP_Post $post    The post object.
		 */
		function clear_batcache_on_post_edit( $post_id, $post ) {
			wp_cache_flush();
		...
		add_action( 'save_post', 'clear_batcache_on_post_edit', 10, 2 );
