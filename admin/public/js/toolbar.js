...
var PCM_TOOLBAR_FLUSH_CACHE = {
	...
	init: function () {
		...
		jQuery( "#wp-admin-bar-pcm-toolbar-parent-default li" ).click(
			function (e) {
		...
		jQuery.ajax(
			{
				...
				success: function (data) {
					if (data.success) {
						...
						pcm_New_Dialog.dialog(
							"pcm-modal-toolbarsettings",
							{
								close: function () {
									pcm_New_Dialog.clone.remove();
								},
								finish: function () {
									jQuery( "#" + pcm_New_Dialog.id ).find( "input[type='checkbox']:checked" ).each(
										function (i, e) {
							...
							function (dialog) {
								jQuery( "#" + pcm_New_Dialog.id ).find( "input[type='checkbox']" ).each(
									function (i, e) {
...
var PCM_TOOLBAR_REMOVE_BRANDING = {
	...
	init: function () {
		...
		jQuery( "#wp-admin-bar-pcm-toolbar-parent-remove-branding-default li" ).click(
			function (e) {
				var id     = (typeof e.target.id != "undefined" && e.target.id) ? e.target.id : jQuery( e.target ).parent( "li" ).attr( "id" );
				var action = "";
